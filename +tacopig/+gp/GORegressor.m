% Generic Observation GP-regression model
%
% The idea behind a GO Regressor is that the inputs do not need
% to be "point" observations. Observations can be over
% a continuous path in the input space. 
% Other more complicated observation types can be added if needed,
% such as surface observations and so on.
% 
% The main difference with a "point only" GP Regresor is that
% X is no longer a matrix, but a struct containing two
% fields: points and paths over the domain space.
% Paths will contain a generic representation of a cubic spline.
% The mean and covariance matrix are concatenated for each observation
% i.e. each element of mu represents one generic observations,
% and each element of K represents the covariance between two observations.

% For this approach to work, covariance functions have to calculate
% the covariance between all combinations of known observation types.
% i.e. the covariance between:
%                               point-point (existing)
%                               point-path
%                               path-path

% To solve the covariances between points and paths,
% we use Integral Kernels [O'Callaghan AIII 2011].

% Example instantiation
% GP = tacopig.gp.GORegressor;
% This creates a instance of a Gaussian process generic observation
% regressor called GP.
classdef GORegressor < tacopig.gp.GpCore
